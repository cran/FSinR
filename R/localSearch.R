...
#' @title Hill-Climbing
#' @description The hc \insertCite{Russell2009}{FSinR} method starts with a certain set of features and in each iteration it searches among its neighbors to advance towards a better solution. The method ends as soon as no better solutions are found
...
hc <- function(data, class, featureSetEval, start=NULL, nneigh=length(data)-1, repeats=1, verbose=FALSE){
...
ts <- function(data, class, featureSetEval, start=NULL, numNeigh=(ncol(data)-1), tamTabuList=5, iter=100, iterNoImprovement=NULL, intensification=NULL, iterIntensification=50, interPercentaje=75, tamIntermediateMemory=5, diversification=NULL, iterDiversification=50, forgetTabuList=TRUE, verbose=FALSE){
  if (attr(featureSetEval, 'kind') == "Individual measure") {
    stop('Only feature set measures can be used');
  }
  
  # Check parameters
  if(numNeigh > ncol(data)) stop('The number of neighbors to consider must be less than or equal to the number of features')
